...
void copy_pixels(
    ...
    for (int i = 0; i < total; ++i) {
        ...
        dst_img->at<VecType>(dest_p.y, dest_p.x) = src_img.at<VecType>(src_p.y, src_p.x);
...
void process_image(const string& input_path, const string& output_path, bool is_encrypt) {
    try {
        ...
        cout << "进度: " << endl;
        if (depth == CV_8U) {
            copy_pixels<cv::Vec4b>(img, &output_img, curve, total, offset, is_encrypt);
        } else if (depth == CV_16U) {
            copy_pixels<cv::Vec4w>(img, &output_img, curve, total, offset, is_encrypt);
        } else {
        ...
        if (ext == ".jpg" || ext == ".jpeg") {
            if (output_img.channels() == 4) {
                // 将 RGBA 转换为 BGR（丢弃 Alpha 通道）
                cvtColor(output_img, output_img, cv::COLOR_RGBA2BGR);
            }
        }
